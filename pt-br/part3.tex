...
\begin{itemize}
\item Você  escreve seu documento em  \texttt{text simples} com \cmd{comandos} que descrevem sua estrutura e significado.
\item O programa \texttt{latex} processa seu texto e seus comandos para produzir um texto formosamente formatado.
\end{itemize}
\vskip 2ex
\begin{center}
...
\begin{itemize}
\item Um comando começa com uma \emph{backslash} \keystrokebftt{\bs}.
\item Alguns comandos podem receber um \emph{argumento} entre chaves \keystrokebftt{\{}
\keystrokebftt{\}}.
\item Alguns comenados também recebem  \emph{argumentos opcionais} enter colchetes \keystrokebftt{[} \keystrokebftt{]}.
\vskip 2ex
...
\begin{itemize}{\small
\item Começa com o comando \cmdbs{documentclass} --- determina o tipo de documento.
\item Metadados (\cmdbs{title} e \cmdbs{author}) e pacotes ficam no 
preâmbulo.
\item O texto propriamente dito fica entre \cmdbegin{document} e \cmdend{document}.
...
\begin{enumerate}
\item Aqui você encontra um texto para um pequeno artigo:\footnote{Based on \url{http://www.cgd.ucar.edu/cms/agu/scientific_talk.html}}
\begin{center}
...
\LaTeX{}.
\item Faz uso da classe de documentos \bftt{beamer}.
\item Usa o ambiente \bftt{frame} para criar slides.
\end{itemize}
\begin{minipage}{0.55\linewidth}
...
\begin{itemize}
\item Use o comando \cmdbs{frametitle} para colocar título nos slide.
...
\item Para gerar o sumário, use o comando \cmdbs{tableofcontents}.  Here's
