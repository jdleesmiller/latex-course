...
\end{minipage}
\begin{minipage}{0.35\linewidth} 
\expandafter\includegraphics\expandafter[width=\textwidth,clip,trim=1.5in 6in 4in 1in]{structure-sections.pdf}
\end{minipage}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Interlúdio: Argumentos de opções}
\begin{itemize}
\item Usamos colchetes  \keystrokebftt{[} \keystrokebftt{]} para inserir 
argumentos de opções, ao invés de chaves\keystrokebftt{\{} \keystrokebftt{\}}.
\item \cmdbs{includgraphics} aceita argumentos de  opçòes que permitem você transformar
...
\item \cmdbs{documentclass} também aceita argumenos de opções. Exemplo:
...
(ela pode  ``flutuar'').
\item Você pode colocar uma legenda para a figura, a qual pode ser referenciada 
 usando o comando \cmdbs{ref}.
\end{itemize}
\begin{minipage}{0.55\linewidth}
\inputminted[fontsize=\scriptsize,frame=single,resetmargins]{latex}%
  {media-graphics.tex}
...
\begin{itemize}
\item Coloque suas referências em um arquivo  \bftt{.bib} no formato do  `bibtex' :
...
usar para referenciá-lo no documento. Por exemplo.  \bftt{Jacobson1999Towards} é
...
\href{https://code.google.com/p/abntex2/}{\bftt{abntex2}} que
...
\begin{itemize}
\item Use o pacote \bftt{natbib} \footnote{Há um novo pacote com mais opções
chamado \bftt{biblatex} porém a maioria dos modelos de artigos ainda utilizam o 
  \bftt{natbib}.} com os comandos \cmdbs{citet} e \cmdbs{citep}.
\item Insira o comando \cmdbs{bibliography} no final do artigo e especifique o
estilo da referência com \cmdbs{bibliographystyle}.
\end{itemize}
...
\begin{center}
\fbox{\href{\fileuri/big_chick.png?dl=1}{Clique para baixar a imagem}}
...
\item (Para encontrar as chaves do arquivo \bftt{.bib}, você terá que abri-lo no seu computador --- \emph{ainda} não é possível vê-lo \emph{online} no  writeLaTeX.)
...
\begin{frame}[fragile]{\insertsubsection}
\begin{itemize}
\item Adicione o comando \cmdbs{tableofcontents} para gerar o sumário automaticamente a partir dos comandos de \cmdbs{section} commands.
...
\item \bftt{cwpuzzle}: para palavras cruzdas
\end{itemize}
...
\begin{frame}{\insertsubsection}
\begin{itemize}
\item Para rodar o  \LaTeX{} no seu próprio computador, você vai precisar de uma \emph{distribuição} do \LaTeX{}. Uma distribuição inclui  um programa \bftt{latex} e (tipicamente) alguns milhares de pacores.
\begin{itemize}
...
\item Google em geral vai levâ-lo para um dos \emph{links} acima.
\end{itemize}
\end{frame}
